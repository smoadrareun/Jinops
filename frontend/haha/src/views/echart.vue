<template>
  <div class="all">
    <div class="top">
      <div class="sign">
        <span class="fast-flicker">b站</span>数据<span class="flicker"
          >分析</span
        >展示
      </div>
    </div>
    <div class="main">
      <div class="up">
        <div class="one" ref="six"></div>
        <div class="two" ref="two"></div>
        <div class="three" ref="three"></div>
      </div>
      <div class="down">
        <div class="one" ref="four"></div>
        <div class="two" ref="five"></div>
        <div class="three" ref="one"></div>
      </div>
    </div>
  </div>
</template>
<script>
import { getdata } from "@/api/demo.js";
export default {
  name: "",
  components: {},
  props: {},
  ...
  methods: {
    getdataM() {
      ...
      getdata().then((res) => {
        this.obj = res.data;
        //4
        this.obj.follower.map((i) => {
          this.uptop.push(i.name);
          //this.ydata.push(i.waterdata)
        });

        //5
        this.obj.level.map((i) => {
          this.level.push({ value: i.count, name: i.level + "级" });
        });
        ...
        this.obj.view.map((i) => {
          ...
          this.bofang.push(i.view);
          //  this.bofang.sort(function(a,b){return a-b});
        });
        this.bofang.reverse();
        this.title.reverse();
        ...
        this.obj.tname.map((i) => {
          this.fenqu.push({ value: i.count, name: i.tname });
        });

        //6

        this.obj.vip.map((i) => {
          this.vip.push({ value: i.count, name: i.vip });
        });
    ...
    setMyEchart1() {
      let myChart = this.$refs.one; //找到要变成echarts的节点
      let thisChart = this.$echarts.init(myChart); //利用原型调取Echarts的初始化方法，将节点变成echart容器
      let option = {
        title: {
          left: "right",
          text: "视频播放量一周趋势图",
        ...
        series: [
          {
            ...
            data: [1215, 5343, 65412, 254613, 284315, 304512, 325095],
            z: 10,
            zlevel: 0,
          },
          {
            ...
            symbolOffset: [1, 1],
            data: [1000, 5000, 60000, 250000, 270000, 300000, 310000],
            width: 2,
            z: 0,
            zlevel: 1,
          },
      ...
      thisChart.setOption(option); //将编写好的配置项挂载到Echarts容器上
    }, //ok

    setMyEchart2() {
      let myChart = this.$refs.two; //找到要变成echarts的节点
      let thisChart = this.$echarts.init(myChart);
      let myColor = [
        "#eb2100",
        "#eb3600",
        "#d0570e",
    ...
    setMyEchart5() {
      let mychart = this.$refs.five;
      let thisChart = this.$echarts.init(mychart);
      let colorList = ["#FE4444", "#FFA611", "#66DA38", "#3FA1FF"];
      let option = {
        backgroundColor: "#032540",
        title: {
          left: "center",
    ...
    setMyEchart6() {
      let myChart = this.$refs.six; //找到要变成echart的节点
      let thisChart = this.$echarts.init(myChart); //利用原型调取Echarts的初始化方法，将节点变成echart容器
      let option = {
...
.all {
  ...
  font-size: 40px;
  z-index: -1;
  background: url(//static.animpen.com/u/3/starsky.jpg) no-repeat;
